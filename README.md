...
의존성이 없는 대신 CI에는 들어가지 않습니다. 모바일 레이아웃을 건드렸다면 직접 돌려보세요. 창 크기 조절 대신 앱을 해당 크기의 iframe에 띄워 재므로 미디어 쿼리는 정확하지만, **실기기의 주소창 높이 변화·터치 동작·iOS Safari 하단 바는 재현하지 못합니다.** `NEXT_PUBLIC_DEV_AUTH_BYPASS=1`로 열면 계정 컨트롤이 렌더되지 않으니, 그 버튼까지 재려면 실제 로그인 세션에서 한 번 더 돌리세요.
