...
지금은 로그인 후 Firestore에 표식 문서를 한 번 읽어 보고, 규칙이 `permission-denied`로 거부하면 로그아웃시킨 뒤 권한 없음을 알립니다. 실제 경계는 언제나 규칙이며, 화면은 그 판단을 그대로 따릅니다. 거부가 아닌 실패(네트워크 오류, 응답 지연 등)는 거부로 보지 않고 세션을 유지하며, 읽기가 10초 안에 끝나지 않으면 기다리지 않고 넘어갑니다.
